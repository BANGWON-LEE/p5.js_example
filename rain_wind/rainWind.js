...
let farRain03 = new Array(7).fill({x:value[4].x_value, y: value[4].y_value });
...
function draw() {
      ...
      for(let i = 0; i<nearRain.length; i++){
         ...
         stroke(0,0,0);
         strokeWeight(2.9);
         ellipse(nearRain[i].x+z, nearRain[i].y + j  , 1, 46);
         strokeWeight(1.6);
         ellipse(farRain[i].x+z, farRain[i].y + j  , 1, 46);
         strokeWeight(2.9);
         ellipse(nearRain02[i].x+z, nearRain02[i].y + j , 1, 38);
         strokeWeight(2.9);
         ellipse(nearRain02[i].x+z, nearRain02[i].y + j  , 1, 38);
         strokeWeight(2.1);
         ellipse(farRain02[i].x+z, farRain02[i].y + j  , 1, 38);
         strokeWeight(2.1);
         ellipse(farRain03[i].x+z, farRain03[i].y + j , 1, 38);
      }
      raining();
...
// nearRain의 속도 조절 및 위치 초기화
   ...
   for(let i = 0; i<nearRain.length; i++){
      ...
      farRain03[i].y = (farRain03[i].y)+speed[2].yspeed
      ...
      if((farRain02[i].y > height) || (farRain02[i].y > 290)){
         // speed[i].yspeed = speed[i].yspeed * -1;
         farRain02[r].x = value[3].x_value
         farRain02[r].y = value[3].y_value
      }
